...
<div style="text-align:left"><span style="color:black; font-size:1em;">Step 1: Load the libraries</span></div>
...
<div style="text-align:left"><span style="color:black; font-size:1em;">Step 2: Get your data</span></div>
...
<div style="text-align:left"><span style="color:black; font-size:1em;">Step 3: Read in your data</span></div>
...
```{r echo=TRUE, , fig.align='left',message=FALSE, warning=FALSE}
...
# head(map@data) - check read in (if necessary)
...
We now have a dataframe that we can use with the (magnificent) ggplot2. Here is an excellent ggplot2 [cheatsheet](https://github.com/rstudio/cheatsheets/blob/master/data-visualization-2.1.pdf) which I have found useful on many occasions. </p>

<div style="text-align:left"><span style="color:black; font-size:1em;">Step 4: ggplot!</span></div>
We can use *geom_polygon* to create our map, and strip out axes, gridlines etc. using  *theme*.

```{r echo=TRUE, message=FALSE, fig.align='center', warning=FALSE, fig.height= 6, fig.width=5}
...
ggplot() +
  ...
  theme(axis.title.x = element_blank(),axis.title.y = element_blank(), 
        text = element_text(color = "#5F5F5F"), 
        plot.title=element_text(size=12, face="bold"),
        panel.grid = element_blank(), # remove all axes and gridlines
        axis.text = element_blank(),
        axis.ticks.x = element_blank(),
        legend.position = "bottom") +
  labs(caption = "Sport England: Active Lives Survey 2017/2018") +
  ggtitle("Adults (16+) who are active", subtitle = "150 Minutes and over per week, by Local Authority")+
